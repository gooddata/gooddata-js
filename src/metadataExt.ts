...
export class MetadataModuleExt {
    ...
    public async saveDashboardAs(
        ...
        try {
            ...
            visWidgetUris.push(...(Array.from(visWidgetMap.values()) as string[]));
            ...
            if (options.copyInsight || typeof options.copyInsight === "undefined") {
                visWidgetUris.map(uri => this.cascadingDelete(projectId, uri));
            } else {
                visWidgetUris.map(uri => this.metadataModule.deleteObject(uri));
            }
            allCreatedObjUris.map(uri => this.cascadingDelete(projectId, uri));
            alert(err);
            return dashboardUri;
        }
    }
            ...
            objsFromDashboard
                ...
                .map(async (kpiWidget: any) => {
                    ...
                    if (options.copyKpi || typeof options.copyKpi === "undefined") {
                        ...
                        uriMap.set(kpi.meta.uri as string, kpi.meta.uri as string);
                    }
                }),
        );
        return uriMap;
